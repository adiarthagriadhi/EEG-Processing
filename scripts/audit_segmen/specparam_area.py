"""Uji cepat per AREA (pasangan kiri+kanan dirata-rata): garis latar & mu periodik (specparam) per fase TURUN/TAHAN/NAIK, semua segmen,
...
FIX, TRIM = 0.85, 0.25
CH = ["Fp1","Fp2","F3","F4","F7","F8","C3","C4","T3","T4","P3","P4","T5","T6","O1","O2"]
...
AREA = {"frontopolar": ["Fp1","Fp2"], "frontal": ["F3","F4"], "frontotemporal": ["F7","F8"], "sentral": ["C3","C4"],
        "temporal": ["T3","T4"], "parietal": ["P3","P4"], "temporo-posterior": ["T5","T6"], "oksipital": ["O1","O2"]}
def spec(x, sf):
    p, f = psd_array_multitaper(x, sf, fmin=1.5, fmax=31, bandwidth=2.5, verbose=False)
    P = np.array([np.interp(GRID, f, pp) for pp in p])
    return np.array([P[[CH.index(c) for c in v]].mean(0) for v in AREA.values()])
def fit(P):
...
for pid in sys.argv[1:]:
    ...
    thr = np.median([w[0] for w in W]); B = np.mean([w[1] for w in W if w[0] <= thr], 0)
    for ph, (c0, c1) in PH.items():
        ...
        if not S: continue
        M = np.mean(S, 0)
        for k, ar in enumerate(AREA):
            b0, f1 = fit(B[k]), fit(M[k])
            out.append(dict(pid=pid, grup=grp[pid], fase=ph, area=ar, garis_latar_dB=10*(f1[0]-b0[0]),
                            mu_periodik_dB=10*(f1[1]-b0[1]), beta_periodik_dB=10*(f1[2]-b0[2])))
d = pd.DataFrame(out); d.to_csv("/tmp/claude-0/sp/area.csv", index=False)
res = []
for (ph, ar), q in d.groupby(["fase", "area"]):
    for c in ["garis_latar_dB", "mu_periodik_dB", "beta_periodik_dB"]:
        a, b = q[q.grup == "penari"][c], q[q.grup == "non-penari"][c]
        res.append(dict(fase=ph, area=ar, ukuran=c.replace("_dB",""), penari=a.mean(), non=b.mean(),
                        d=(a.mean()-b.mean())/np.sqrt((a.var()+b.var())/2), p=stats.ttest_ind(a, b, equal_var=False)[1]))
r = pd.DataFrame(res)
from statsmodels.stats.multitest import multipletests
r["p_fdr"] = multipletests(r.p, method="fdr_bh")[1]
print(r[r.p < 0.1].sort_values("p").round(2).to_string(index=False))
print("jumlah uji", len(r))
